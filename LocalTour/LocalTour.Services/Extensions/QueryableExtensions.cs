...
namespace LocalTour.Services.Extensions
{
    public static class QueryableExtensions
    {
        ...
        private static bool IsValidProperty<TEntityDto>(string propertyName)
        {
            ...
            {
                // Log or print available properties for debugging
                var properties = typeof(TEntityDto).GetProperties(BindingFlags.Public | BindingFlags.Instance)
                    .Select(p => p.Name).ToList();
                Console.WriteLine($"Invalid sortBy property: {propertyName}. Available properties: {string.Join(", ", properties)}");
            }
